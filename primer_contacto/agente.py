...
# Obtener la ruta base del módulo primer_contacto
PRIMER_CONTACTO_DIR = os.path.dirname(os.path.abspath(__file__))
...
class PersistentMCPClient:
    ...
    async def _run_mcp_server(self):
        ...
        # Buscar servidor_mcp.py en la carpeta primer_contacto/
        servidor_path = os.path.join(PRIMER_CONTACTO_DIR, "servidor_mcp.py")
        params = StdioServerParameters(command="python", args=[servidor_path])
        try:
...
def setup_crew(modo: str = "local") -> Crew:
    ...
    def cargar_prompt(nombre_archivo: str, default: str) -> str:
        ruta = os.path.join(PRIMER_CONTACTO_DIR, "system_prompts", nombre_archivo)
        if os.path.exists(ruta):
            with open(ruta, "r", encoding="utf-8") as f:
                return f.read().strip()
        return default

    prompt_clasificador = cargar_prompt("clasificador.md", "Eres un Clasificador de Modelos de Negocio experto.")
    ...
    reglas_generales = ""
    ruta_reglas = os.path.join(PRIMER_CONTACTO_DIR, "context", "reglas_auditor.md")
    ...
    clasificador = Agent(
        role="Clasificador de Modelo de Negocio",
        goal="Identificar y clasificar el modelo operativo de {negocio} en {ciudad}",
        backstory=prompt_clasificador,
        llm=llm,
        verbose=True,
    ...
    tarea_clasificacion = Task(
        description=(
            "Analiza el negocio '{negocio}' en '{ciudad}'.\n"
            "Modelo sugerido por el usuario: '{modelo_negocio}'\n"
            "Contexto de la empresa: '{contexto_empresa}'\n\n"
            "Determina el modelo de negocio (local, delivery, ecommerce, saas, profesional, marketplace), "
            "evalúa qué plataformas son críticas y cuáles no aplican. Responde en JSON válido."
        ),
        expected_output="JSON estructurado con la clasificación del modelo de negocio, prioridades y no_aplica.",
        agent=clasificador
    )
    ...
    tarea_supervision = Task(
        description=(
            "Revisa la clasificación del modelo de negocio, la investigación, la auditoría y el reporte de competencia.\n"
            ...
            "IMPORTANTE: Adapta el score y las recomendaciones al modelo de negocio clasificado. "
            "Por ejemplo, si es e-commerce, Google Maps no aplica y no debe restar puntos. Si es delivery, prioriza apps externas."
        ),
        ...
        context=[tarea_clasificacion, tarea_investigacion, tarea_auditoria, tarea_competencia]
    )
    ...
    return Crew(
        agents=[clasificador, investigador_marca, auditor_huella, analista_competencia, estratega_supervisor],
        tasks=[tarea_clasificacion, tarea_investigacion, tarea_auditoria, tarea_competencia, tarea_supervision],
        verbose=True,
        memory=usar_memoria,
